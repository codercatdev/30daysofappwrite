...
<script>
	...
	const fetchMemberships = () => api.getMemberships(params.id).then((r) => r.memberships);
	const createMembership = (email, name) =>
		api.createMembership(params.id, email, ['member'], `${window.origin}/#/acceptMembership`, name);
